...
window.addEventListener('load', () => {
    ...
    // == VEHICLE PARAMETERS AS PER PROMPT SPECIFICATIONS ==
    const VEHICLE_PARAMS = {
        CHASSIS_MASS: 120, // kg
        REAR_BAR_DIM: { w: 1.2, h: 0.25 },
        FRONT_BAR_DIM: { w: 0.8, h: 0.25 },
        FRONT_BAR_OFFSET: { x: 0.8, y: 0.15 },
        WHEEL_MASS: 12, // kg
        WHEEL_RADIUS: 0.35, // m
        WHEEL_FRICTION: 1.5, // Standard friction for slip calculation
        WHEEL_RESTITUTION: 0.1,
        TRACK_WIDTH: 1.25,
        
        // Suspension physics
        SUSPENSION_FREQ_HZ: 6.0, 
        SUSPENSION_DAMPING_RATIO: 0.8,
        SUSPENSION_TRAVEL: 0.20,
        MAX_SPRING_FORCE: 50000, // Increased force for more robust suspension

        // Engine, brake, and control physics
        MOTOR_TORQUE: 550, // Max torque in N·m
        MOTOR_MAX_SPEED: 55, // Max motor speed in rad/s
        BRAKE_TORQUE: 1200, // N·m
        ENGINE_BRAKE_TORQUE: 50, // N·m
        AIR_CONTROL_TORQUE: 120, // N·m
        AIR_CONTROL_DAMPING: 80, // Damping factor to prevent overly fast spins
    ...
    function handleContact(contact, isBeginning) {
        ...
        const dataA = getContactData(contact.getFixtureA());
        const dataB = getContactData(contact.getFixtureB());
        if (!dataA || !dataB) return;

        const checkPair = (d1, d2, fix2) => {
            if (d1.type === 'wheel' && d2.type === 'ground') d1.owner.setGrounded(d1.wheelId, isBeginning);
            if (d1.type === 'chassis' && d2.type === 'checkpoint' && isBeginning) {
                gameState.lastCheckpoint = { pos: vehicle.chassis.getPosition(), angle: vehicle.chassis.getAngle(), linearVel: vehicle.chassis.getLinearVelocity(), angularVel: vehicle.chassis.getAngularVelocity() };
                world.destroyBody(fix2.getBody());
            }
            if (d1.type === 'chassis' && d2.type === 'fuel' && isBeginning) {
                gameState.fuel = Math.min(GAME_PARAMS.FUEL_START, gameState.fuel + 50);
                world.destroyBody(fix2.getBody());
            }
        };
        checkPair(dataA, dataB, contact.getFixtureB());
        checkPair(dataB, dataA, contact.getFixtureB());
    }
    ...
    function createTerrainManager() {
        let bodies = [], lastGeneratedX = 0;
        const seed = Math.random() * 1000;
        TERRAIN_PARAMS.P1 = seed; TERRAIN_PARAMS.P2 = seed + 100;
        ...
        const generateSegment = (startX) => {
            const points = []; let lastY = getHeight(startX);
            ...
            for (let x = startX + TERRAIN_PARAMS.SAMPLE_DISTANCE; x <= startX + TERRAIN_PARAMS.SEGMENT_LENGTH; x += TERRAIN_PARAMS.SAMPLE_DISTANCE) {
                let y = getHeight(x); const slope = (y - lastY) / TERRAIN_PARAMS.SAMPLE_DISTANCE;
                if (Math.abs(slope) > TERRAIN_PARAMS.MAX_SLOPE) y = lastY + Math.sign(slope) * TERRAIN_PARAMS.MAX_SLOPE * TERRAIN_PARAMS.SAMPLE_DISTANCE;
                points.push(Vec2(x, y)); lastY = y;
            }
            const body = world.createBody(Vec2.zero());
            body.createFixture(pl.Chain(points, false), { friction: TERRAIN_PARAMS.FRICTION, restitution: TERRAIN_PARAMS.RESTITUTION, userData: { type: 'ground' } });
            ...
            for(let i = 1; i < points.length - 1; i++) {
                if (points[i].y > points[i-1].y && points[i].y > points[i+1].y) {
                    const x = points[i].x;
                    if (x > (terrainManager.lastCheckpointX + GAME_PARAMS.CHECKPOINT_DISTANCE)) { createCollectible(Vec2(x, points[i].y + 1.5), 'checkpoint'); terrainManager.lastCheckpointX = x; }
                    if (x > (terrainManager.lastFuelX + GAME_PARAMS.FUEL_CAN_DISTANCE)) { createCollectible(Vec2(x, points[i].y + 1.5), 'fuel'); terrainManager.lastFuelX = x; }
                }
            }
        };
        return {
            ...
            getSlope(x) {
                ...
                world.rayCast(Vec2(x, 50), Vec2(x, -50), (fixture, point, n) => {
                    ...
                    if (userData && userData.type === 'ground') {
                        normal = n;
                        return 0;
                    }
                    return -1;
    ...
    function createVehicle(world, pos) {
        ...
        const rearWheelAnchor = Vec2(-TRACK_WIDTH / 2, -1.0);
        const frontWheelAnchor = Vec2(TRACK_WIDTH / 2, -1.0);

        const createWheelAssembly = (wheelId, localAnchorOnChassis) => {
            const worldPos = chassis.getWorldPoint(localAnchorOnChassis);
            const wheel = world.createDynamicBody({ position: worldPos, bullet: true });
            wheel.createFixture(pl.Circle(WHEEL_RADIUS), { ...wheelFixtureDef, userData: { type: 'wheel', wheelId: wheelId, owner: null } });
            // The PrismaticJoint allows the wheel to move up and down for suspension.
            // We removed the conflicting RevoluteJoint that locked the suspension in place. The motor
            // is now simulated by applying torque directly to the wheel body.
            const suspension = world.createJoint(pl.PrismaticJoint({ localAnchorA: localAnchorOnChassis, localAnchorB: Vec2.zero(), localAxisA: Vec2(0, 1), enableLimit: true, lowerTranslation: -VEHICLE_PARAMS.SUSPENSION_TRAVEL, upperTranslation: VEHICLE_PARAMS.SUSPENSION_TRAVEL, }, chassis, wheel));
            return { wheel, suspension };
        };

        const rearAssembly = createWheelAssembly('rear', rearWheelAnchor);
        const frontAssembly = createWheelAssembly('front', frontWheelAnchor);
        ...
        const vehicleObj = {
            ...
            reset(pos, angle, linearVel, angularVel) {
                this.chassis.setPosition(pos); this.chassis.setAngle(angle);
                this.chassis.setLinearVelocity(linearVel || Vec2.zero()); this.chassis.setAngularVelocity(angularVel || 0);
                this.rearWheel.setPosition(chassis.getWorldPoint(rearWheelAnchor)); this.frontWheel.setPosition(chassis.getWorldPoint(frontWheelAnchor));
                this.rearWheel.setLinearVelocity(linearVel || Vec2.zero()); this.frontWheel.setLinearVelocity(linearVel || Vec2.zero());
                this.rearWheel.setAngularVelocity(0); this.frontWheel.setAngularVelocity(0);
            },

            // =========================================================================
            // == NEW VEHICLE PHYSICS UPDATE FUNCTION (PROMPT IMPLEMENTATION) =========
            // =========================================================================
            update(dt, input) {
                
                // --- 1. SUSPENSION PHYSICS ---
                const updateSuspension = (suspensionJoint, wheel) => {
                    const { SUSPENSION_FREQ_HZ, SUSPENSION_DAMPING_RATIO, MAX_SPRING_FORCE, CHASSIS_MASS, WHEEL_MASS } = VEHICLE_PARAMS;
                    const m_eff = (CHASSIS_MASS / 2) + WHEEL_MASS;
                    const omega = 2 * Math.PI * SUSPENSION_FREQ_HZ;
                    const k = omega * omega * m_eff;
                    const c = 2 * SUSPENSION_DAMPING_RATIO * omega * m_eff;
                    const x = suspensionJoint.getJointTranslation();
                    const v = suspensionJoint.getJointSpeed();
                    let forceMag = -k * x - c * v;
                    forceMag = clamp(forceMag, -MAX_SPRING_FORCE, MAX_SPRING_FORCE);
                    const axis = this.chassis.getWorldVector(Vec2(0, 1));
                    const force = axis.mul(forceMag);
                    const wheelPos = wheel.getPosition();
                    wheel.applyForce(force, wheelPos, true);
                    this.chassis.applyForce(force.mul(-1), this.chassis.getWorldPoint(suspensionJoint.m_localAnchorA), true);
                };

                updateSuspension(this.rearSuspension, this.rearWheel);
                updateSuspension(this.frontSuspension, this.frontWheel);

                // --- 2. AIR CONTROL PHYSICS ---
                if (!this.isRearGrounded && !this.isFrontGrounded) {
                    ...
                    const userInputTorque = -input.pitch * AIR_CONTROL_TORQUE;
                    const dampingTorque = -currentOmega * AIR_CONTROL_DAMPING; 
                    this.chassis.applyTorque(userInputTorque + dampingTorque);
                }
                
                // --- 3. ENGINE AND BRAKING PHYSICS (REIMPLEMENTED) ---
                const { MOTOR_TORQUE, MOTOR_MAX_SPEED, BRAKE_TORQUE, ENGINE_BRAKE_TORQUE, WHEEL_RADIUS } = VEHICLE_PARAMS;
    
                // Calculate effective torque based on wheel slip for the rear drive wheel
                let effectiveMotorTorque = MOTOR_TORQUE;
                if (this.isRearGrounded && input.throttle > 0) {
                    const omega = this.rearWheel.getAngularVelocity(); // rad/s
                    const vx = this.chassis.getLinearVelocity().x; // m/s
                    const tangentialSpeed = -omega * WHEEL_RADIUS; // omega is negative for forward motion.
                    const slipRatio = (tangentialSpeed - vx) / Math.max(Math.abs(vx), 0.1);
                    effectiveMotorTorque = MOTOR_TORQUE * (1 - 0.3 * clamp(Math.abs(slipRatio), 0, 1));
                    
                    if (Math.abs(slipRatio) > 0.3) {
                        const wheelPos = this.rearWheel.getPosition();
                        const particlePos = Vec2(wheelPos.x, wheelPos.y - WHEEL_RADIUS);
                        const particleVel = this.chassis.getLinearVelocity().clone().mul(0.2);
                        particleVel.x -= slipRatio * 1.5;
                        spawnParticle(particlePos, particleVel, 0.7, '150,150,150');
                    }
                }

                // --- Apply torque to Rear (Drive) Wheel ---
                let rearTorque = 0;
                const rearOmega = this.rearWheel.getAngularVelocity();
                if (input.brake > 0 && this.isRearGrounded) {
                    rearTorque = clamp(-rearOmega * 100, -BRAKE_TORQUE, BRAKE_TORQUE);
                } else if (input.throttle > 0) {
                    if (rearOmega > -MOTOR_MAX_SPEED) {
                         rearTorque = -effectiveMotorTorque;
                    }
                } else if (this.isRearGrounded) {
                    rearTorque = clamp(-rearOmega * 10, -ENGINE_BRAKE_TORQUE, ENGINE_BRAKE_TORQUE);
                }
                this.rearWheel.applyTorque(rearTorque, true);

                // --- Apply torque to Front Wheel (Braking Only) ---
                let frontTorque = 0;
                const frontOmega = this.frontWheel.getAngularVelocity();
                if (input.brake > 0 && this.isFrontGrounded) {
                    frontTorque = clamp(-frontOmega * 100, -BRAKE_TORQUE, BRAKE_TORQUE);
                }
                this.frontWheel.applyTorque(frontTorque, true);
            }
        };
        rearAssembly.wheel.getFixtureList().getUserData().owner = vehicleObj;
        frontAssembly.wheel.getFixtureList().getUserData().owner = vehicleObj;
        return vehicleObj;
    }
    ...
    // I. UI / HUD
    const hud = { speed: document.getElementById('speed-value'), rpm: document.getElementById('rpm-value'), fuel: document.getElementById('fuel-value'), distance: document.getElementById('distance-value'), slope: document.getElementById('slope-value'), gameOverPanel: document.getElementById('game-over-panel'), update() { const chassisPos = vehicle.chassis.getPosition(); const linearVel = vehicle.chassis.getLinearVelocity().length(); const rpm = Math.abs(vehicle.rearWheel.getAngularVelocity() * 60 / (2 * Math.PI)); const slope = terrainManager.getSlope(chassisPos.x); gameState.distance = Math.max(gameState.distance, chassisPos.x); this.speed.textContent = (linearVel * 3.6).toFixed(0); this.rpm.textContent = rpm.toFixed(0); this.fuel.textContent = gameState.fuel.toFixed(0); this.distance.textContent = gameState.distance.toFixed(1); this.slope.textContent = (slope * 100).toFixed(0); if(gameState.gameOver) this.gameOverPanel.classList.remove('hidden'); } };
